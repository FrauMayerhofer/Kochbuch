...
class GerichteScreen extends React.Component {
  ...
  openErstellenScreen(){
    ...
    this.props.navigation.navigate('Erstellen', {type: "GERICHT"})
  }
  ...
  sort(selectedOption){
    ...
    if(selectedOption==="standard"){
      this.setState({...this.state, filter: false, filterData: this.props.items, isPickerVisible: false})
    }
  }
  ...
  render() {
    return (
        ...
        <FlatList
          data={this.state.isSearching ? this.state.gesuchteItems : this.state.filter ? this.state.filterData : this.props.items}
          renderItem={({item, index}) => 
          <TouchableOpacity style={styles.items} onPress={() =>{ 
            this.props.selectItem("SELECT_GERICHT", item.key)
            this.props.navigation.navigate('Detail',{type: "GERICHT"})} }>
            {item.image !== "" ? <Image style={styles.image} source={{uri: item.image}} resizeMode={"cover"}/> : null}
...
export default connect(mapStateToProps, mapDispatchToProps)(GerichteScreen);